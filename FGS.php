...
<script>
    ...
    function DocumentStart()
    {
        ...
        LayoutMain = new dhtmlXLayoutObject({
            ...
            cells: [
                {id: "a", header: true, text:"Accessory Material Total List"},
                {id: "b", header: true, text:"Control Panel", width: 300}
            ]
        });
        ToolbarMain.addText("TitleText", null, '<textarea  style="height:0px;width:0" id="ClipBoard"/>');
        ToolbarMain.addButton("CopyData", null, "Copy Data", "save.gif");
        ToolbarMain.attachEvent("onClick", function(name) {
            if(name == "CopyData"){
                var DataCB = "";
                for(var i = 0; i < GridMainTotal.getColumnsNum(); i++) DataCB += "\t" + GridMainTotal.getColLabel(i);
                DataCB +=  "\r\n";
                var rowCount = GridMainTotal.getRowsNum();
                for (var i=0; i < rowCount; i++) {
                    var id = GridMainTotal.getRowId(i);
                    GridMainTotal.forEachCell(id,function(cellObj,ind){
                        DataCB = DataCB + "\t" + GridMainTotal.cells(id,ind).getValue();
                    });
                    DataCB = DataCB + "\r\n";
                }
    ...
    var RowID,RowLot,ColLot;
    var ControlForm;
    var GridMainTotal;
    var dp;

    function AddRowGrid(LastRow){
        var D = (new Date()).valueOf();
        GridMain.addRow(UserString + D,[LastRow + 1]);
    ...
    function InitGrid(){
        ...
        GridMain.setHeader("NO,GLID, FGS_IN_OUT, JOB_ID, Record_Date, Record_Name, Remark");
        GridMain.attachHeader(",#text_filter,,,#text_filter,,");
        GridMain.setInitWidths("50,150,100,100,100,100,*")
        GridMain.setColAlign("center,center,center,center,center,center,center");
        GridMain.setColTypes("ro,ed,ed,ed,ed,ed,ed");
        ...
        GridMain.init();
        GridMain.loadXML("Data.php?EVENT=LOADFGS",function(){
            var LastRow = GridMain.getRowsNum();
            if(LastRow == 0 || GridMain.cells2(LastRow - 1,3).getValue() != "") {
                AddRowGrid(LastRow);
            }
        });
        dp = new dataProcessor("Data.php?EVENT=LOADFGS"); 
        dp.init(GridMain);
        ...
        GridMainTotal = LayoutMain.cells("b").attachGrid();
        GridMainTotal.setImagePath("./Module/dhtmlx/skins/skyblue/imgs/");
        GridMainTotal.setHeader("NO,GLID, JOB_ID");
        GridMainTotal.attachHeader(",#text_filter,");
        GridMainTotal.setInitWidths("50,150,*")
        GridMainTotal.setColAlign("center,center,center");
        GridMainTotal.setColTypes("ro,ro,ro");
        GridMainTotal.setColSorting("str,str,str")
        GridMainTotal.setRowTextStyle("1", "background-color: red; font-family: arial;");
        GridMainTotal.enableSmartRendering(true);
        GridMainTotal.entBox.id = "GridMainTotal";
        GridMainTotal.enableBlockSelection(true);
        GridMainTotal.init();
        GridMainTotal.loadXML("Data.php?EVENT=LOADFGSTOTAL");
    }
