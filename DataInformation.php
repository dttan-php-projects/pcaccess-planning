...
<script>
    ...
    function DocumentStart()
    {
        ...
        ToolbarMain.addButton("MLA", null, "MLA", "save.gif");
        ToolbarMain.addButton("RBO", null, "RBO Request", "save.gif");
        ToolbarMain.addButton("Customer", null, "Customer Request", "save.gif");
        ...
        ToolbarMain.attachEvent("onClick", function(id) {
            if (id == "MLA") window.location = "?P=MLA";
            else if (id == "RBO") window.location = "?P=RBO";
            else if (id == "Customer") window.location = "?P=Customer";
        });
        InitGrid();
    ...
    function MLAGrid(){
        ...
        GridMain.setHeader("STT,RBO,MLA,REMARK");
        GridMain.setInitWidths("50,100,50,*")
        ...
        GridMain.init();
        GridMain.load("Data.php?EVENT=LOADMLA",function(){
            var LastRow = GridMain.getRowsNum();
            if(GridMain.cells(GridMain.getRowId(LastRow - 1),1).getValue() != "") {
                var D = (new Date()).valueOf();
                GridMain.addRow(D,[D]);
            }
        });
        dp = new dataProcessor("Data.php?EVENT=LOADMLA");
        dp.init(GridMain);
    }

    function CustomerGRID(){
        GridMain.setImagePath("./Module/dhtmlx/skins/skyblue/imgs/");
        GridMain.setHeader("STT,RBO, BILL TO, SHIP TO, Remark Drill, Remark 1, Remark 2");
        GridMain.setInitWidths("50,150,250,250,200,*,*")
        GridMain.setColAlign("center,center,center,center,center,center,center");
        GridMain.setColTypes("ro,ed,ed,ed,ed,ed,ed");
        GridMain.setColSorting("str,str,str,int,str,str,str")
        GridMain.setRowTextStyle("1", "background-color: red; font-family: arial;");
        GridMain.entBox.id = "GridMain";
        GridMain.enableBlockSelection(true);
        GridMain.init();
        GridMain.load("Data.php?EVENT=LOADCUSTOMERREQUEST",function(){
            var LastRow = GridMain.getRowsNum();
            if(GridMain.cells(GridMain.getRowId(LastRow - 1),1).getValue() != "" || 
            GridMain.cells(GridMain.getRowId(LastRow - 1),2).getValue() != "" || 
            GridMain.cells(GridMain.getRowId(LastRow - 1),3).getValue() != "" || 
            GridMain.cells(GridMain.getRowId(LastRow - 1),4).getValue() != "") {
                var D = (new Date()).valueOf();
                GridMain.addRow(D,[D]);
            }
        });
        dp = new dataProcessor("Data.php?EVENT=LOADCUSTOMERREQUEST");
        dp.init(GridMain);
    }

    function RBOGRID(){
        GridMain.setImagePath("./Module/dhtmlx/skins/skyblue/imgs/");
        GridMain.setHeader("STT,RBO, Type RBO, Remark Drill, Remark 1, Remark 2");
        GridMain.setInitWidths("50,150,250,200,*,*")
        GridMain.setColAlign("center,center,center,center,center,center");
        GridMain.setColTypes("ro,ed,ed,ed,ed,ed");
        GridMain.setColSorting("str,str,str,str,str,str")
        GridMain.setRowTextStyle("1", "background-color: red; font-family: arial;");
        GridMain.entBox.id = "GridMain";
        GridMain.enableBlockSelection(true);
        GridMain.init();
        GridMain.load("Data.php?EVENT=LOADRBOREQUEST",function(){
            var LastRow = GridMain.getRowsNum();
            if(LastRow == 0 || GridMain.cells2(LastRow - 1,3).getValue() != "") {
                AddRowGrid(LastRow);
            }
        });
        dp = new dataProcessor("Data.php?EVENT=LOADRBOREQUEST");
        dp.init(GridMain);
    }

    function InitGrid(){
        GridMain = LayoutMain.cells("a").attachGrid();
        if(PageView == null || PageView == "MLA") {
            MLAGrid();
        } else if(PageView == "RBO") {
            RBOGRID();
        } else if(PageView == "Customer") {
            CustomerGRID();
        }
