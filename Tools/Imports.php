                    ...
                    for ($i = 2; $i <= count($allDataInSheet); $i++) {
                        ...
                        // check automail
                            ...
                            if (!empty($automailData) ) {
                                ...
                                // Xử lý theo yêu cầu của Dung.Phan: Nếu đã tồn tại thì thêm mới nhưng vẫn lấy cột ISSUE_ORDER giá trị cũ
                                // Lấy dữ liệu ISSUE_ORDER cũ để insert cái mới
                                $dataCheck = MiQuery( "SELECT * FROM $table_receiving WHERE `ORDER_NUMBER`='$OrderNumber' AND `LINE_NUMBER`='$LineNumber' ORDER BY `ID` DESC LIMIT 1;", _conn());
                                if (!empty($check ) ) {
                                    $ISSUE_ORDER = $dataCheck[0]['ISSUE_ORDER'];
                                } else {
                                    $ISSUE_ORDER = $ISSUE;
                                }
                                ...
                                $Result = MiNonQuery( "INSERT INTO $table_receiving
                                                        ...
                                                        VALUES
                                                            ( '$OrderNumber', '$LineNumber', '$SOLINE', '$CUSTOMERPO', '$ORDERED_ITEM', '$ITEM', '$QTY', '$UOM', '$REQUEST_DATE', '$PROMISE_DATE', '$SHIP_TO_CUSTOMER', '$BILL_TO_CUSTOMER', NOW(), '$ISSUE_ORDER', '$CUSTOMER_REQUEST');", 
                                                        _conn() 
                                                    );
